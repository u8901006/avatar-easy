  ...
  nodes: {
    ...
    r1: {
      ...
      tip: {
        ...
        clinical:
          "批評性聲音的語言特徵包括全稱否定（global devaluation）與絕對化思維，常用絕對的字眼宣判，彷彿其判斷即為事實。在更嚴重的形式裡，這些話會固化為「惡性核心陳述」（malignant kernel statements）——被體驗為絕對真理的僵化自我結論（van der Hart et al., 2006）。介入初期應協助辨識這些認知扭曲，而非直接挑戰其內容（Mosquera & Ross, 第8章、第16章）。",
      },
      choices: [
        {
    ...
    r3_recognized: {
      ...
      tip: {
        ...
        clinical:
          "批評性聲音的保護功能通常是「先發制人」——以內在的批評取代外界可能的批評，以減少被拒絕時的痛苦。它深信「吃苦才能換來安全」「感到羞恥就能免於更大的傷害」，有時更是出於對嚴厲照顧者的忠誠。理解這層保護意圖是轉化的關鍵。此時聲音開始從「敘事位置」滑向「經驗位置」，透露自己也曾經歷被批判的痛苦（Mosquera & Ross, 第8章、第16章）。",
      },
      choices: [
        {
    ...
    r4_reveal: {
      ...
      tip: {
        ...
        clinical:
          "此時可開始與聲音協商：它的保護意圖值得肯定，但方式需要調整。運用「反映距離」（reflective distance）邀請成人自我觀察：「聽起來有一個部分相信你不配得到關懷，我想知道它在保護什麼。」也可以詢問：「如果這個部分不必對你如此嚴苛，會發生什麼事？」——這能溫和地命名批判底下的恐懼，而非與其內容爭辯（Mosquera & Ross, 第16章）。",
      },
      choices: [
        {
    ...
    r5_negotiate: {
      ...
      tip: {
        ...
        clinical:
          "這是聲音轉化的關鍵時刻。隨著持續、同調的臨在，僵化的批判最終能轉化為辨識力（discernment）、界線設定，或「堅定而溫和的關懷」（fierce compassion）。這些部分其實蘊含寶貴特質——警覺、正直、敢說真話、誠實與力量；一旦能量不再被禁錮在「懲罰」之中，就能重新被整合進內在系統（Mosquera & Ross, 第16章）。",
      },
      choices: [
        {
    ...
    r5_resistant: {
      ...
      tip: {
        ...
        clinical:
          "批評性聲音的忠誠面向往往被忽視。它雖然嚴厲，卻是少數「始終在場」的內在聲音，其堅持本身就是一種忠誠與被認可的渴望。這類部分常測試治療師的穩定，不是想衝突，而是預期被拒絕——因為過去「事情變困難時，從來沒有人留下來」。帶著同等的尊重與慈悲，同時容納「遭受攻擊的」與「發動攻擊的」兩個部分，光是這姿態本身就是一種矯正性經驗（corrective experience）（Mosquera & Ross, 第16章）。",
      },
      choices: [
        {
