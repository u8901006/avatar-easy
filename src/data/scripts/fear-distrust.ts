  ...
  nodes: {
    ...
    "voice-silence-1": {
      ...
      tip: {
        ...
        clinical:
          "恐懼與不信任的部分源於基本信任受損的早期經驗，其溝通模式是「不可見性」——透過讓自己消失來避免危險。它有外顯（凍住、麻木）與隱蔽（「有禮貌的離開」：閒聊、表面參與、話題一親密就轉開）兩種面貌，後者極易被忽略。介入時避免任何壓迫，並明確給予「允許暫停」的許可——這反而會讓它軟化（創傷知情人格部分工作臨床手冊，第10章；Focus topic：恐懼與迴避）。",
      },
      choices: [
        {
    ...
    "r6-visible": {
      ...
      tip: {
        ...
        clinical:
          "此時聲音正在經歷「可見性悖論」——最渴望的同時也是最恐懼的。每一次它多停留一秒、多吐露一句，都是「感到安全的微小瞬間」（micro-moments of safety）。這些時刻應被溫和地承認，但絕不過度放大或強調，否則它會覺得被看光、被暴露，於是又縮回去。讓它感受到「被看見是安全的」是治療的里程碑（創傷知情人格部分工作臨床手冊；Focus topic：恐懼與迴避）。",
      },
      choices: [
        {
